...
class CrepusMobileRuntime(
    ...
    private fun refreshFromDevServer() {
        runCatching {
            ...
            val envelope = decodeHotReloadEnvelope(jsonText)
            val current = ir.value
            if (current != null) {
                runCatching { current.apply(envelope) }.getOrElse { fetchFullIr() }
            } else {
                fetchFullIr()
            }
        }.onSuccess { loaded ->
    ...
    private fun fetchFullIr(): ViewIr {
        val connection = URL("$baseUrl/ir").openConnection() as HttpURLConnection
        connection.connectTimeout = 500
        connection.readTimeout = 1000
        return connection.inputStream.bufferedReader().use { decodeViewIr(it.readText()) }
    }
