...
public final class CrepusMobileRuntime: ObservableObject {
    ...
    private func refreshFromDevServer() async {
        do {
            let (eventData, _) = try await URLSession.shared.data(from: baseURL.appendingPathComponent("events"))
            let envelope = try JSONDecoder().decode(HotReloadEnvelope.self, from: eventData)
            if var current = ir {
                do {
                    try current.apply(envelope)
                    ir = current
                } catch {
