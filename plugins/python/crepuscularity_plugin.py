...
def _crepus_bin() -> str:
    ...
    if "/" in bin_path or "\\" in bin_path:
        raise ValueError("Invalid CREPUS_BIN: must be strictly a binary name without directory separators")
    if bin_path not in ("crepus", "crepus.exe"):
        raise ValueError(f"Invalid CREPUS_BIN: binary name must be 'crepus' or 'crepus.exe', got {bin_path!r}")
    return bin_path
