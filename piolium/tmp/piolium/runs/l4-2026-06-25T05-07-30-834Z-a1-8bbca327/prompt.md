# Run l4-2026-06-25T05-07-30-834Z-a1-8bbca327
Agent: probe-strategist
Source: /Users/undivisible/.pi/agent/npm/node_modules/@vigolium/piolium/agents/probe-strategist.md
...
Steps you must complete in order, all in one session:
  1. Read `piolium/attack-surface/knowledge-base-report.md`. If absent, fail with a clear note.
  2. Read `piolium/attack-surface/candidates-summary.md` and pick one or two highest-impact attack-surface slices.
  3. Write `piolium/attack-surface/manual-attack-surface-inventory.md` with entry points, public routes/URLs, attacker sources, sinks, hidden control channels, middleware/proxy assumptions, and key files.
  ...
  7. Write `piolium/attack-surface/balanced-probe-summary.md` summarising the probe execution and coverage.
...
- Target repository: /Users/undivisible/projects/crepuscularity
...
`piolium/probe-workspace/<component>/code-anatomy.md`.
...
```
Attack surface map: piolium/probe-workspace/<component>/attack-surface-map.md
Code anatomy: piolium/probe-workspace/<component>/code-anatomy.md
Layer trust chain gaps: [paste the Trust Chain Gaps section]
Output file: piolium/probe-workspace/<component>/round-1-hypotheses.md
```
...
Code anatomy: piolium/probe-workspace/<component>/code-anatomy.md
Layer trust chain gaps: [paste the Trust Chain Gaps section]
Output file: piolium/probe-workspace/<component>/round-2-hypotheses.md
```
...
For each match, write a cross-model seed to `piolium/probe-workspace/<component>/cross-model-seeds.md`:
...
```
Hypotheses files:
  - piolium/probe-workspace/<component>/round-1-hypotheses.md
  - piolium/probe-workspace/<component>/round-2-hypotheses.md
Cross-model seeds: piolium/probe-workspace/<component>/cross-model-seeds.md
Component source paths: [from attack surface map]
Output file: piolium/probe-workspace/<component>/round-1-evidence.md
```
...
Needs-Deeper: <N>
Stop reason: <reason>
Summary: piolium/probe-workspace/<component>/probe-summary.md
```