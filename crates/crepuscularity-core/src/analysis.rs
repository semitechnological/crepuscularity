...
mod tests {
    ...
    fn test_analyze_template() {
        let nodes = vec![
            ...
            // 1: Dynamic text (1 expr)
            Node::Text(vec![TextPart::Expr("name".to_string())]),
            // 2: Static element
            ...
            // 3: Dynamic element (event handler makes it dynamic, 1 expr)
            Node::Element(Element {
                ...
                event_handlers: vec![EventHandler {
                    ...
                    modifiers: vec![],
                    handler: "say_hi()".to_string(),
                }],
                bindings: vec![],
                animations: vec![],
                children: vec![],
            }),
            // 4: If block (always dynamic, 1 expr for condition)
            Node::If(IfBlock {
                condition: "is_ready".to_string(),
                then_children: vec![],
                else_children: None,
            }),
        ];
        ...
        assert_eq!(map.dynamic_indices, vec![1, 3, 4]);
        assert_eq!(map.expr_count, 3);
    }
}
