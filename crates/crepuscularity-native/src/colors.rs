//! Full Tailwind v3 named color palette.
//!
//! Usage: `lookup_named_color("slate-500")` → `Some("#64748b")`

/// Look up a Tailwind named color like `"slate-500"` or `"red-100"`.
/// Returns the hex string (with `#` prefix) or `None` if not found.
...
///
/// Resolution order:
/// 1. Basic CSS named colors (`red`, `blue`, `green`, `white`, `black`,
///    `gray`/`grey`, `clear`/`transparent`, `orange`, `yellow`, `purple`, `pink`)
/// 2. Full Tailwind v3 palette names (`red-500`, `slate-700`, etc.) via [`lookup_named_color`]
/// 3. Hex literals: `#rrggbb` (alpha = 1.0) and `#rrggbbaa`
///
/// Returns `None` for unrecognised strings. Callers should handle semantic tokens like
/// `"primary"` and `"secondary"` themselves before calling this function.
pub fn resolve_rgba(css: &str) -> Option<[f32; 4]> {
    ...
    // Try Tailwind palette (e.g. "red-500" → "#ef4444")
...
mod tests {
    ...
    fn tailwind_named() {
        ...
        assert!((r - 0x64 as f32 / 255.0).abs() < 0.01);
        assert!((g - 0x74 as f32 / 255.0).abs() < 0.01);
        assert!((b - 0x8b as f32 / 255.0).abs() < 0.01);
        assert_eq!(a, 1.0);
    }
    ...
    fn unknown_returns_none() {
        ...
        assert!(resolve_rgba("secondary").is_none());
