use gpui::{TestAppContext, WindowOptions, Render, AppContext};
...
fn test_apply_window_deferred(cx: &mut TestAppContext) {
    ...
    let _ = bridge.invoke_envelope("window", "setTitle", &serde_json::json!({ "title": "New Title 1" }));
    let _ = bridge.invoke_envelope("window", "setTitle", &serde_json::json!({ "title": "New Title 2" }));
    let _ = bridge.invoke_envelope("window", "setContentSize", &serde_json::json!({ "width": 800.0, "height": 600.0 }));
    let _ = bridge.invoke_envelope("window", "setDocumentEdited", &serde_json::json!({ "edited": true }));
    let _ = bridge.invoke_envelope("window", "requestDecorations", &serde_json::json!({ "style": "client" }));
    let _ = bridge.invoke_envelope("window", "requestDecorations", &serde_json::json!({ "style": "server" }));
