...
## Legacy `site.json` pipeline

The **`--legacy-site-json`** flag and the old **single-file HTML** output mode were **removed**. Migrate using the steps above; **`crepus web build`** rejects **`--legacy-site-json`** (see [`cli.md`](cli.md)).

## Release note summary

**`crepus web build`** is **`.crepus`-first** with a **WASM runtime** aligned with **`crepus webext`**; **`site.json`** is **optional shell metadata only** (title, SEO, theme variables).
